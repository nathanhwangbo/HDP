...
def datetimes_to_windows(datetimes: np.ndarray, window_radius: int) -> np.ndarray:
    ...
    time_index = (
        np.zeros(
            (
                len(day_of_yr_to_index),
                np.max([len(x) for x in day_of_yr_to_index.values()]),
            ),
            int,
        )
        - 1
    ...
    window_samples = np.zeros(
        (len(day_of_yr_to_index), 2 * window_radius + 1, time_index.shape[1]), int
    )
    ...
    return window_samples.reshape(
        (window_samples.shape[0], window_samples.shape[1] * window_samples.shape[2])
    )


@nb.guvectorize(
    [(nb.float32[:], nb.int64[:, :], nb.float64[:], nb.float64[:, :])],
    "(t), (d, b), (p) -> (d, p)",
)
...
def compute_percentiles_wrapper(baseline_data, rolling_windows, percentiles):
    threshold_da = xarray.apply_ufunc(
        compute_percentiles,
        baseline_data,
        rolling_windows,
        percentiles,
...
def compute_threshold(
    ...
    rolling_windows_indices = datetimes_to_windows(
        baseline_data.time.values, rolling_window_size
    )
    rolling_windows_coords = {
        "doy": np.arange(rolling_windows_indices.shape[0]),
        "t_index": np.arange(rolling_windows_indices.shape[1]),
    }
    rolling_windows = xarray.DataArray(
        data=rolling_windows_indices, coords=rolling_windows_coords
    )

    percentiles = xarray.DataArray(data=percentiles, coords={"percentile": percentiles})
    ...
    da_coords = {
        coord: baseline_data.coords[coord]
        for coord in baseline_data.coords
        if coord != "time"
    }
    ...
    threshold_da = xarray.map_blocks(
        ...
        obj=baseline_data,
        kwargs={"rolling_windows": rolling_windows, "percentiles": percentiles},
        template=template,
    )
    ...
    if "long_name" in threshold_da.attrs:
        add_history(
            threshold_da,
            f"Metadata updated: 'long_name' value '{threshold_da.attrs['long_name']}' overwritten by HDP.\n",
        )
    ...
    ds["doy"].attrs = dict(
        units="day_of_year",
        baseline_calendar=str(baseline_data.time.values[0].calendar),
    )
    return ds


def compute_thresholds(
    baseline_dataset: list[xarray.DataArray],
    percentiles: np.ndarray,
    no_season: bool = False,
    rolling_window_size: int = 7,
    fixed_value: float = None,
...
def compute_threshold_io(
    ...
    threshold_ds = compute_threshold(
        baseline_data, percentiles, no_season, rolling_window_size, fixed_value
    )
